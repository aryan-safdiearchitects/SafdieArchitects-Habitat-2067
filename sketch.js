...
function draw() {
  ...
  if (kaleidoscopeMode) {
    ...
    // Draw scanlines for 80s CRT effect
    drawScanlines();
  ...
  for (let i = eqParticles.length - 1; i >= 0; i--) {
    ...
    if (p.isRadial) {
      ...
      if (p.life > 0 && p.radius < maxDist) {
        ...
        // Rotate square to align with radial direction
        push();
        translate(p.x, p.y);
...
function updateAuraModeColumns(bassNorm, midNorm, trebleNorm, overallAmp, time) {
  ...
  // Spawn new EQ particles from active columns
  for (let i = 0; i < numColumns; i++) {
    let col = columns[i];
    ...
    // Spawn rate based on energy - higher energy = more particles
    let spawnChance = energy * 0.3;
    ...
    if (energy > 0.25 && random() < spawnChance && eqParticles.length < 800) {
      ...
      // Y position at top of the displaced column
      let y = imgY + (habitatImg.height / 2 + col.currentY) * imgScale - imgH * 0.1;

      // Sample color from the image at this column position
      let imgPx = floor(col.x + col.width / 2);
      let imgPy = floor(habitatImg.height * 0.4); // Sample from upper portion
      imgPx = constrain(imgPx, 0, habitatImg.width - 1);
      imgPy = constrain(imgPy, 0, habitatImg.height - 1);
      let c = habitatImg.get(imgPx, imgPy);
      ...
      // If too dark, use synthwave colors based on region
      if (r + g + b < 150) {
        if (region === 0) {
          r = colors.magenta[0]; g = colors.magenta[1]; b = colors.magenta[2];
        } else if (region === 1) {
      ...
      eqParticles.push({
        ...
        vy: -2 - energy * 3, // Upward velocity based on energy
        size: particleSize,
        life: 1,
        color: [r, g, b],
        energy: energy,
